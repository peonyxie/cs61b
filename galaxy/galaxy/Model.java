...
class Model {
    ...
    /**
     * create vertical bond.
     */
    static final char VERTBOND = 'I';

    /**
     * create vertical edge.
     */
    static final char VERTEDGE = '|';

    /**
     * create horizontal bond.
     */
    static final char HORIBOND = '=';

    /**
     * create horizontal edge.
     */
    static final char HORIEDGE = '-';

    /**
     * create intersection.
     */
    static final char INTERSEC = '+';

    /**
     * create center.
     */
    static final char CENTER = 'o';

    /**
     * create center with boundary.
     */
    static final char CENTERBOUNDARY = 'O';
     ...
     * init center list.
     */
    private ArrayList<Place> centers = new ArrayList<>();
    ...
    void copy(Model model) {
        ...
        for (int m = 0; m < r * 2 + 1; m++) {
            for (int n = 0; n < c * 2 + 1; n++) {
                this.board[m][n] = model.board[m][n];
            }
        }
        for (int x = 0; x < r; x++) {
            for (int y = 0; y < c; y++) {
                this.value[x][y] = model.value[x][y];
            }
        }
    ...
    void init(int cols, int rows) {
        ...
        this.c = cols;
        this.r = rows;
        char[][] ans = new char[2 * rows + 1][2 * cols + 1];
        for (int x = 0; x < 2 * cols + 1; x++) {
            for (int y = 0; y < 2 * rows + 1; y++) {
                if (x % 2 == 0 && (x == 0 || x == 2 * cols)) {
                    ans[y][x] = VERTBOND;
                } else if (y % 2 == 0 && (y == 0 || y == 2 * rows)) {
                    ans[y][x] = HORIBOND;
                } else if (y % 2 == 0 && x % 2 == 0) {
                    ans[y][x] = INTERSEC;
                } else if (y % 2 == 0 && !(x == 0) && !(x == 2 * rows)) {
                    ans[y][x] = HORIEDGE;
                } else if (x % 2 == 0 && !(y == 0) && !(y == 2 * cols)) {
                    ans[y][x] = VERTEDGE;
                } else {
                    ans[y][x] = EMPTYCELL;
                    mark(x, y, 0);
    ...
    boolean isCenter(int x, int y) {
        ...
        return this.board[y][x] == CENTER || this.board[y][x] == CENTERBOUNDARY;
    }
    ...
    boolean isBoundary(int x, int y) {
        return this.board[y][x] == CENTERBOUNDARY
                || this.board[y][x] == HORIBOND || this.board[y][x] == VERTBOND;
    }
    ...
    boolean solved() {
        int total;
        total = 0;
        for (Place cen : centers()) {
            HashSet<Place> reg = findGalaxy(cen);
            if (reg == null) {
                return false;
            } else {
                total += reg.size();
    ...
    private void accreteRegion(Place cell, HashSet<Place> region) {
        ...
        for (int i = 0; i < 4; i += 1) {
            int dx = (i % 2) * (2 * (i / 2) - 1),
                    dy = ((i + 1) % 2) * (2 * (i / 2) - 1);
            Place newCell = cell.move(2 * dx, 2 * dy);
            if (!isBoundary(cell.move(dx, dy)) && !region.contains(newCell)) {
                accreteRegion(newCell, region);
            }
        }
    }
    ...
    private boolean isGalaxy(Place center, HashSet<Place> region) {
        for (Place cell : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                int dx = (i % 2) * (2 * (i / 2) - 1),
                        dy = ((i + 1) % 2) * (2 * (i / 2) - 1);
                Place fakeBoundary = cell.move(dx, dy),
                        nextCell = cell.move(2 * dx, 2 * dy);
                if ((isCenter(fakeBoundary) && !fakeBoundary.equals(center))
                        || (region.contains(nextCell)
                        && isBoundary(fakeBoundary))) {
                    return false;
                }
            }
            for (int i = 0; i < 4; i += 1) {
                ...
                if (!center.equals(intersection) && isCenter(intersection)) {
                    return false;
                }
            }
        }
    ...
    HashSet<Place> findGalaxy(Place center) {
        ...
        if (isCell(center)) {
            ...
            init = center.move(1, 1);
        } else if (isVert(center)) {
            init = center.move(1, 0);
        } else {
            init = center.move(0, 1);
        }
        HashSet<Place> galaxy = new HashSet<>();
        accreteRegion(init, galaxy);
    ...
    Set<Place> maxUnmarkedRegion(Place center) {
        ...
        for (Place near : unmarkedContaining(center)) {
            regionlist.add(near);
        }
        while (unmarkedSymAdjacent(center, regionlist).size() != 0) {
            for (Place cell : unmarkedSymAdjacent(center, regionlist)) {
                regionlist.add(cell);
                markAll(regionlist, 1);
            }
        }
        HashSet<Place> region = new HashSet(regionlist);
        markAll(region, 0);
        return region;
    }
    ...
    void markGalaxies(int v) {
        assert v >= 0;
        markAll(0);
        for (Place cent : centers()) {
            HashSet<Place> region = findGalaxy(cent);
            if (region != null) {
                markAll(region, v);
            }
        }
    ...
    void toggleBoundary(int x, int y) {
        if (isCenter(x, y)) {
            this.board[y][x] = CENTERBOUNDARY;
        } else if (isEdge(x, y)) {
            if (!isBoundary(x, y)) {
                if (isHoriz(x, y)) {
                    this.board[y][x] = HORIBOND;
                } else {
                    this.board[y][x] = VERTBOND;
                }
            } else {
                if (y % 2 == 0) {
                    this.board[y][x] = HORIEDGE;
                } else {
                    this.board[y][x] = VERTEDGE;
                }
            }
        }
    }
    ...
    void placeCenter(Place p) {
        if (isBoundary(p)) {
            this.board[p.y][p.x] = CENTERBOUNDARY;
        } else {
            this.board[p.y][p.x] = CENTER;
        }
        centers.add(p);
    }
    ...
    void markAll(int v) {
        assert v >= 0;
        for (int x = 1; x < this.cols() * 2; x += 2) {
            for (int y = 1; y < this.rows() * 2; y += 2) {
                mark(x, y, v);
            }
        }
    }
    ...
    Place opposing(Place p0, Place p) {
        if (isCell(p.x, p.y)
                && isCell(p0.x + (p0.x - p.x), p0.y + (p0.y - p.y))) {
            return pl(p0.x + (p0.x - p.x), p0.y + (p0.y - p.y));
        }
        return null;
    }
    ...
    List<Place> unmarkedContaining(Place place) {
        ...
        } else if (x % 2 == 0 && y % 2 == 1 && !(y == 0) && !(y == 2 * r)) {
            if (mark(x + 1, y) == 0 && (mark(x - 1, y) == 0)) {
                return asList(place.move(-1, 0), place.move(1, 0));
            }
        } else if (y % 2 == 0 && x % 2 == 1 && !(x == 0) && !(x == 2 * c)) {
            if (mark(x, y + 1) == 0 && (mark(x, y - 1) == 0)) {
                return asList(place.move(0, -1), place.move(0, 1));
            }
        } else if (y % 2 == 0 && x % 2 == 0) {
            if (mark(x + 1, y + 1) == 0 && (mark(x - 1, y - 1) == 0)
                    && (mark(x - 1, y + 1) == 0
                    && (mark(x + 1, y - 1) == 0))) {
    ...
    List<Place> unmarkedSymAdjacent(Place center, List<Place> region) {
        ...
        for (Place regi : region) {
            assert isCell(regi);
            for (int i = 0; i < 4; i += 1) {
                int dx = 2 * ((i % 2) * (2 * (i / 2) - 1)),
                        dy = 2 * (((i + 1) % 2) * (2 * (i / 2) - 1));
                Place p = regi.move(dx, dy);
                Place oppo = opposing(center, p);
                if (oppo != null) {
                    if (mark(p) == 0 && (mark(oppo) == 0)) {
                        result.add(p);
                    }
                }
            }
