...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("auto\\sblack$", this::doAutoblack),
        new Command("auto\\swhite$", this::doAutowhite),
        new Command("manual\\swhite$", this::doManualwhite),
        new Command("manual\\sblack$", this::doManualblack),
        new Command("(\\w\\d+)(\\W\\w\\d+)([(]\\w\\d+[)])", this::doMove),
        new Command("(\\w\\d+)(\\s\\w\\d+)((\\s\\w\\d)+)", this::doMove),
    ...
    /** Move on the board on standard output.
     * @param mat pattern passed in.
     */
    private void doMove(Matcher mat) {
        String from = mat.group(1);
        String to = mat.group(2);
        String spear = mat.group(3);
        Square f = Square.sq(String.valueOf
                (from.charAt(0)), from.substring(1));
        Square t = Square.sq(String.valueOf
                (to.charAt(1)), to.substring(2));
        Square s = Square.sq(String.valueOf
                (spear.charAt(1)), spear.substring(2, spear.length() - 1));
        try {
            _board.makeMove(f, t, s);
        } catch (UnknownError exp) {
    ...
    /** Command "manualblack".
     * @param unsued pattern passed in
     */
    private void doManualblack(Matcher unsued) {
